function new_img = stitch_multiple_images2(imgs)
    ...
    [~, num_inliers12] = get_transform(x1, y1, x2, y2);
    [x1, y1, x3, y3] = get_matches(imgray1, imgray3, do_visualization);
    [~, num_inliers13] = get_transform(x1, y1, x3, y3);
    [x2, y2, x3, y3] = get_matches(imgray2, imgray3, do_visualization);
    [~, num_inliers23] = get_transform(x2, y2, x3, y3);
    if (num_inliers23 < num_inliers12 && num_inliers23 < num_inliers13)
        % situation 1: 2,3 cannot be stitched together
        [img31, ~, inlier_position31] = stitch_images_2(img3, img1); 
        [img312, ~, inlier_position12, T, xmin, ymin] = stitch_images_2(img31, img2);
...
%         scatter(inlier_position12(:,1),inlier_position12(:,2),'go'); hold on;
    ...
    elseif (num_inliers13 < num_inliers12 && num_inliers13 < num_inliers23)
        ...
        [img12, ~, inlier_position12] = stitch_images_2(img1, img2);
        [img123, ~, inlier_position23, T, xmin, ymin] = stitch_images_2(img12, img3);
...
%         scatter(inlier_position23(:,1),inlier_position23(:,2),'go'); hold on;
        ...
        [img13, ~, inlier_position13] = stitch_images_2(img1, img3);
        [img132, ~, inlier_position32, T, xmin, ymin] = stitch_images_2(img13, img2);       
